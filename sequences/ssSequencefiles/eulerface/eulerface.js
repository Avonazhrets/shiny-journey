;(function(global) {
  ...
  mathMVC.startDrag = function(callback) {
    return function(event) {
      ...
      while (!/mmvce/.test(target.className)) target = target.parentNode
      mathMVC.activeId = target.id
      mathMVC.target =
        target.childElementCount === 2 ? target.lastChild : target
  ...
  mathMVC.drag = function(event) {
    var change =
      (event.clientX - mathMVC.origin.x + (mathMVC.origin.y - event.clientY)) /
      mathMVC.coeff
    mathMVC.updatePosition(event)
    mathMVC.models[mathMVC.activeId] = String(
      (Number(mathMVC.models[mathMVC.activeId]) + change).toFixed(2),
    )
  ...
  mathMVC.select = function(id, field, divs, models, callback) {
    ...
    dropper.addEventListener('click', function() {
      ...
      if (style.visibility === 'hidden') dropDown.style.visibility = 'visible'
  ...
  mathMVC.getModelById = function(id) {
    ...
    return mathMVC.models[id].replace(/\$([a-zA-Z0-9\-_]+)/g, function(
      dummy,
      id,
    ) {
  ...
  mathMVC.makeSelects = function() {
    ...
    for (var i = selects.length - 1; i >= 0; i--)
      mathMVC.elements[selects[i].getAttribute('id')] = new mathMVC.Select(
        selects[i],
      )
  }
